...
function StepReview({ s, set, togglePos, toggleNeg, onSave }) {
  ...
  const apply = async () => {
    set({ step: 4, results: [], dissimilar: [] });
    const [sim, dis] = await Promise.all([
      api.search([...s.classifierPos], [...s.classifierNeg], 200, s.searchMode),
      api.searchDissimilar([...s.classifierPos], 20),
    ]);
  ...
  const ResultCard = ({ r }) => (
    <div style={S.resultCard}>
      ...
      <div style={{ display: "flex" }}>
        <button onClick={() => togglePos(r.object_id)} style={S.acceptBtn}>Similar</button>
        <button onClick={() => toggleNeg(r.object_id)} style={S.rejectBtn}>Dissimilar</button>
      </div>
    </div>
  );
    ...
    <section style={S.card}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div>
          ...
          <p style={{ ...S.muted, margin: 0 }}>
            {s.classifierPos.size} similar{s.classifierNeg.size > 0 ? ` · ${s.classifierNeg.size} dissimilar` : ""} · {visible.length} results
          </p>
        </div>
      ...
      {/* Similar/Dissimilar exemplars */}
      <div style={{ display: "flex", gap: 16, margin: "12px 0 20px", flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#2563EB", marginBottom: 8 }}>Similar ({posCrops.length})</div>
        ...
        {negCrops.length > 0 && (
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#DC2626", marginBottom: 8 }}>Dissimilar ({negCrops.length})</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 8 }}>
              {negCrops.map(c => (
                <div key={c.object_id} style={{ ...S.resultCard, borderColor: "#DC2626", borderWidth: 2 }}>
                  <div style={S.thumbContainer}>
        ...
        <>
          <div style={S.gallerySection}>
            <div style={S.gallerySimilar}>
              <h3 style={S.galleryTitleGreen}>Most Similar ({visible.length})</h3>
              <div style={{ display: "grid", gridTemplateColumns: `repeat(${COLS}, 1fr)`, gap: 10 }}>
                {visible.map(r => <ResultCard key={r.object_id} r={r} />)}
              </div>
              {visible.length === 0 && (
                <p style={S.emptyMsg}>No results. Try adding more exemplars.</p>
              )}
            </div>
          </div>

          {s.dissimilar.length > 0 && (
            <div style={{ ...S.gallerySection, marginTop: 16 }}>
              <div style={S.galleryDissimilar}>
                <h3 style={S.galleryTitleRed}>Most Dissimilar ({s.dissimilar.length})</h3>
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${COLS}, 1fr)`, gap: 10 }}>
                  {s.dissimilar.map(r => <ResultCard key={r.object_id} r={r} />)}
                </div>
              </div>
            </div>
          )}
